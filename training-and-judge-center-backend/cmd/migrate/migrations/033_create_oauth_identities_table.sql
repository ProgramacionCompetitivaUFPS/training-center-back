...
CREATE TABLE IF NOT EXISTS oauth_identities (
    ...
    provider          VARCHAR(20) NOT NULL CHECK (provider <> ''),
...
-- +goose Down
-- Fails if any user has password IS NULL (OAuth-only accounts). Remove or
-- migrate those users before rolling back this migration.
