...
func newRunningSubmission(t *testing.T) *submission.Submission {
	...
	s, err := submission.NewSubmission(testSubmissionID, testProblemID, shared.RestoreUserID(testUserID), nil, lang, "gs://bucket/code.cpp", testNow)
	if err != nil {
...
func TestNewSubmission_EmptyID_ReturnsError(t *testing.T) {
	lang := submission.RestoreLanguage("cpp20")
	_, err := submission.NewSubmission("", testProblemID, shared.RestoreUserID(testUserID), nil, lang, "gs://bucket/code.cpp", testNow)
	if err == nil {
...
func TestSubmission_Start_FromPending_Succeeds(t *testing.T) {
	lang := submission.RestoreLanguage("cpp20")
	s, _ := submission.NewSubmission(testSubmissionID, testProblemID, shared.RestoreUserID(testUserID), nil, lang, "path", testNow)
