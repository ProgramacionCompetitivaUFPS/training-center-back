...
func (uc *SubmitSolutionUseCase) Execute(ctx context.Context, in SubmitSolutionInput) (*SubmitSolutionOutput, error) {
	...
	isDup, err := uc.submissionRepo.ExistsByHashAndUserAndProblem(ctx, fileHash, userID, problem.ID)
	if err != nil {
		return nil, err
	}
	if isDup {
	...
	last, err := uc.submissionRepo.FindLastByUserAndProblem(ctx, userID, problem.ID)
	if err != nil {
