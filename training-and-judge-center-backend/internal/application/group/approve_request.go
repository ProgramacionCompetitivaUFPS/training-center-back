...
func (uc *ApproveRequestUseCase) Execute(ctx context.Context, input ApproveRequestInput) (*ApproveRequestOutput, error) {
	...
	if err := uc.txManager.WithTx(ctx, func(ctx context.Context) error {
		...
		newMember, err := domainGroup.NewGroupMember(uuid.New().String(), input.GroupID, req.RequesterUserID(), domainGroup.MemberRoleMember, nil, domainGroup.JoinMethodRequestApproved, nil)
		if err != nil {
			return err
		}
