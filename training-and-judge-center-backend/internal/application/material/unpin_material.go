...
func NewUnpinMaterial(
	repo domainMaterial.Repository,
	groupProvider GroupProvider,
	memberProvider GroupMemberProvider,
	authorProvider AuthorProvider,
) *UnpinMaterial {
	return &UnpinMaterial{
		repo:           repo,
		groupProvider:  groupProvider,
...
func (uc *UnpinMaterial) Execute(ctx context.Context, in UnpinMaterialInput) (*UnpinMaterialOutput, error) {
	...
	if !m.CanBePinnedBy(shared.RestoreUserID(in.CurrentUser.ID), in.CurrentUser.IsAdmin(), isGroupLead) {
		return nil, apperror.NewForbidden(ErrCodeInsufficientPerms, "only group leads can unpin materials")
	}
