...
const testCheckerKey = "problems/abc/checker/compiled"
...
func newTestOutputChecker(t *testing.T, docker *mockDockerExecClient, reader gcsReader) (*OutputChecker, *mockPoolDockerClient) {
	t.Helper()
	p, poolDocker := newTestPool(t)
	return &OutputChecker{pool: p, docker: docker, reader: reader, cfg: testArtifactCfg()}, poolDocker
}

func beginTestChecking(t *testing.T, c *OutputChecker) appjudge.CheckerSession {
	t.Helper()
	s, err := c.BeginChecking(context.Background(), testCheckerKey, submission.RestoreLanguage(testLang))
	if err != nil {
...
// would tie one up per judging for nothing. Step 7 turns this branch into a
// claim of the compare language.
func TestOutputChecker_BeginChecking_NoCheckerPathClaimsNoContainer(t *testing.T) {
	c, poolDocker := newTestOutputChecker(t, &mockDockerExecClient{}, storedArtifact("ELF binary"))

	session, err := c.BeginChecking(context.Background(), "", submission.Language{})
	if err != nil {
		t.Fatalf("BeginChecking: %v", err)
	}
	defer session.Close(context.Background())

	result, err := session.Check(context.Background(), appjudge.CheckRequest{
...
// testlib binds argv[2] to the contestant's output and argv[3] to the jury's
// answer. Swapped, a malformed contestant output makes the checker declare the
// jury wrong instead of rejecting the submission.
func TestCheckerSession_Check_PassesTheThreeFilesInTestlibOrder(t *testing.T) {
	...
	if string(written["input.txt"]) != "1 2" {
		t.Errorf("input.txt: got %q, want the test case input", written["input.txt"])
	}
	if string(written["output.txt"]) != "3 " {
		t.Errorf("output.txt: got %q, want the contestant's output", written["output.txt"])
	}
	if string(written["answer.txt"]) != "3" {
		t.Errorf("answer.txt: got %q, want the jury's answer", written["answer.txt"])
	}

	if len(cmds) != 1 || len(cmds[0]) != 3 || cmds[0][0] != "sh" || cmds[0][1] != "-c" {
		t.Fatalf("expected one command through sh -c, got: %v", cmds)
	}
	want := "timeout --kill-after=1s 30s /sandbox/Checker /sandbox/input.txt /sandbox/output.txt /sandbox/answer.txt"
	if cmds[0][2] != want {
...
func TestCheckerSession_Check_RejectionIsAResultNotAnError(t *testing.T) {
	...
	if result.Message != "wrong answer: expected 3, found 4" {
		t.Errorf("message: got %q, want the checker's stderr", result.Message)
	}
}
...
func TestCheckerSession_Check_ReusesOneContainerForEveryTestCase(t *testing.T) {
	...
	c, poolDocker := newTestOutputChecker(t, &mockDockerExecClient{}, reader)
	s := beginTestChecking(t, c)

	for range 3 {
		if _, err := s.Check(context.Background(), appjudge.CheckRequest{}); err != nil {
			t.Fatalf("Check: %v", err)
		}
	}
...
func TestOutputChecker_BeginChecking_MissingArtifactClaimsNoContainer(t *testing.T) {
	...
	_, err := c.BeginChecking(context.Background(), testCheckerKey, submission.RestoreLanguage(testLang))
...
func TestOutputChecker_BeginChecking_RejectsALanguageWithNoArtifactConfig(t *testing.T) {
	...
	c := &OutputChecker{pool: p, docker: &mockDockerExecClient{}, reader: storedArtifact("jar"), cfg: testArtifactCfg()}

	_, err := c.BeginChecking(context.Background(), testCheckerKey, submission.RestoreLanguage("java17"))
