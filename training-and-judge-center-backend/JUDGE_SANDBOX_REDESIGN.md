...
**Y el `terraform plan` de control encontró drift que un apply a ciegas habría deshecho**: el `default-pool` está declarado con `node_count = 2` fijo y alguien lo bajó a 0 a mano para no pagarlo mientras el cluster está apagado. O sea que es drift **deliberado y permanente**, no un accidente, y cualquier `terraform apply` sin acotar lo revierte encendiendo dos nodos. Vale saberlo antes de aplicar por otro motivo. (Acá se aplicó a propósito, porque el cluster tenía que encenderse igual para probar.)
